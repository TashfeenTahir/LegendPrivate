resource "azurerm_network_interface" "NetworkDev" {
  count                 = 4 //"${var.Count}"
  name                = "muks-org-d-nic-${count.index}"
  location            = "${azurerm_resource_group.HubRG.location}"
  resource_group_name = "${azurerm_resource_group.HubRG.name}"

  ip_configuration {
    name                          = "muks-org-d-ipconfig-${count.index}"
    subnet_id                     = "${azurerm_subnet.Dev-APP1-Subnet.id}"  //"${azurerm_subnet.MGMTSubnet-ID.id}"
    private_ip_address_allocation = "Dynamic"
  }
}
resource "azurerm_virtual_machine" "Dev-VM" {
 count                 = 4 //"${var.Count}"
 name                  = "muks-org-d-${count.index}"
 location              = "${azurerm_resource_group.Dev-RG.location}"
 #availability_set_id   = "${azurerm_availability_set.avset.id}"
 resource_group_name   = "${azurerm_resource_group.Dev-RG.name}"
 network_interface_ids = ["${element(azurerm_network_interface.NetworkDev.*.id, count.index)}"]
 ...
 storage_os_disk {
   name              = "muks-org-d-storage-${count.index}"
   caching           = "ReadWrite"
...
*/
 os_profile {
   computer_name  = "muks-org-d-${count.index}"
